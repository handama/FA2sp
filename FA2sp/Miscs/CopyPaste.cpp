...
void CopyPaste::Paste(int X, int Y, int nBaseHeight, MyClipboardData* data, size_t length,
     int recordType, std::set<MapCoord>* validCoords, bool bPasteArea)
{
    if (!bPasteArea)
    ...
    for (const auto& cell : cells)
    {
        ...
        if (CIsoViewExt::PasteOverlays)
        {
            ...
            CMapDataExt::NewOverlay[Y + offset_y + (X + offset_x) * CMapDataExt::X_PLUS_Y_LIMIT] = cell.Overlay;
...
void CopyPaste::PasteArea(int X, int Y, int nBaseHeight, MyClipboardData* data, size_t length, int recordType)
{
    ...
    {
        Paste(coord.X, coord.Y, nBaseHeight, data, length, recordType, recordCoords.get(), true);
    }
