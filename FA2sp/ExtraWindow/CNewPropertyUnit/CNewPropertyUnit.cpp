...
CNewPropertyUnit::CNewPropertyUnit()
{
}
...
BOOL CALLBACK CNewPropertyUnit::DlgProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    switch (Msg)
    {
    case WM_INITDIALOG:
    {
        CNewPropertyUnit* pThis = reinterpret_cast<CNewPropertyUnit*>(lParam);
    ...
    case WM_COMMAND:
    {
        ...
        if (id >= 1300 && id <= 1310 && code == BN_CLICKED)
        {
            bool checked = SendMessage(GetDlgItem(hWnd, id), BM_GETCHECK, 0, 0);
            int idx = id - 1300;
            if (idx >= 0 && idx < 11)
                CViewObjectsExt::VehicleBrushBools[idx] = checked;
        }
...
BOOL CNewPropertyUnit::OnInitDialog(HWND hDlg)
{
    m_hWnd = hDlg;

    FString buffer;

    SetWindowTextA(hDlg, Translations::TranslateOrDefault("UnitCap", "Unit Options"));
    ...
    HWND hLongDesc = GetDlgItem(hDlg, 1285);
    ...
    if (hState)
    {
        ...
        if (!CString_State.IsEmpty())
        {
            FString key = "FootClassStatus.";
            key += CString_State;
            int idx = vcb->FindStringExact(Translations::TranslateOrDefault(key, CString_State));
            if (idx != CB_ERR)
                vcb->SetCurSel(idx);
        }
    ...
    // AboveGround (1085) - ComboBox (0/1)
    HWND hAbove = GetDlgItem(hDlg, 1085);
    if (hAbove)
    {
        auto vcb = std::make_unique<VirtualComboBoxEx>();
        ...
        vcb->AddString("0");
        vcb->AddString("1");
        if (!CString_OnBridge.IsEmpty())
        {
            int index = vcb->FindStringExact(CString_OnBridge);
            if (index != CB_ERR)
                vcb->SetCurSel(index);
            else
                vcb->SetEditText(CString_OnBridge);
        }
        m_comboBoxes[hAbove] = std::move(vcb);
    }

    // FollowsIndex (1086) - ComboBox ("-1 - None")
    HWND hFollow = GetDlgItem(hDlg, 1086);
    if (hFollow)
    {
        auto vcb = std::make_unique<VirtualComboBoxEx>();
        ...
        if (!CString_FollowerID.IsEmpty())
        {
            int index = vcb->FindStringExactStart(CString_FollowerID + " ");
            if (index != CB_ERR)
                vcb->SetCurSel(index);
            else
                vcb->SetEditText(CString_FollowerID);
        }
    ...
    // AutoNORecruitType (1089) - ComboBox (0/1)
    HWND hAutoNO = GetDlgItem(hDlg, 1089);
    if (hAutoNO)
    {
        auto vcb = std::make_unique<VirtualComboBoxEx>();
    ...
    // AutoYESRecruitType (1090) - ComboBox (0/1)
    HWND hAutoYES = GetDlgItem(hDlg, 1090);
    if (hAutoYES)
    {
        auto vcb = std::make_unique<VirtualComboBoxEx>();
    ...
    else
    {
        ...
        {
            HWND hCheck = GetDlgItem(hDlg, 1300 + i);
            if (hCheck && CViewObjectsExt::VehicleBrushBools[i])
...
void CNewPropertyUnit::OnOK(HWND hDlg)
{
    CollectResults(hDlg);
...
void CNewPropertyUnit::OnCancel(HWND hDlg)
{
    ExtraWindow::RestoreDisabledWindows();
...
void CNewPropertyUnit::CollectResults(HWND hDlg)
{
    char buffer[256] = {};
    ...
    if (hAbove && m_comboBoxes[hAbove])
        CString_OnBridge = m_comboBoxes[hAbove]->GetSelectedText(true);

    HWND hFollow = GetDlgItem(hDlg, 1086);
    if (hFollow && m_comboBoxes[hFollow])
    {
        CString_FollowerID = m_comboBoxes[hFollow]->GetSelectedText(true);
        STDHelpers::TrimIndex(CString_FollowerID);
    }
...
void CNewPropertyUnit::UpdateHealthDisplay(HWND hDlg)
{
    char buffer[32] = {};
...
void CNewPropertyUnit::SyncHealthDisplay(HWND hDlg, bool updateText)
{
    if (updateText)
    {
