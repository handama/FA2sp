...
#### `save_map([filepath], [previewOption])`
- **说明**：保存当前地图。不传参数时保存到当前地图路径（若地图尚未保存过、没有路径，则报错并返回失败）；传入 `filepath` 则保存到指定位置。
- **参数**：
  - `filepath` (`string`, 可选) — 保存路径。省略时保存到当前地图路径。
  - `previewOption` (`number`, 可选, 默认 `0`)：
    - `0`：重新生成预览图；
    - `1`：保留现有预览图；
    - `2`：生成隐藏占位预览图（最快）。
- **返回** (`boolean`)：是否保存成功。失败原因会输出到控制台。
- **用法**：
```lua
save_map()                          -- 保存到当前地图路径
save_map("D:\\Maps\\test.map")      -- 保存到指定路径
save_map("D:\\Maps\\test.map", 0)   -- 指定路径并重新生成预览图
```

#### `load_map(filepath)`
- **说明**：加载指定路径的地图，等同于通过编辑器打开地图。注意：当前地图的未保存修改会丢失，脚本扫描会将此函数列为高风险操作。
- **参数**：
  - `filepath` (`string`) — 地图文件路径。
- **返回** (`boolean`)：是否成功发起加载。文件不存在或地图检查器运行中时会报错并返回失败。
- **用法**：
```lua
load_map("D:\\Maps\\test.map")
```
